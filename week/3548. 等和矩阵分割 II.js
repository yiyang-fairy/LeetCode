...
const canPartitionGrid = function (grid) {
  ...
  let total = 0
  grid.forEach(item => total += getSum(item));
  ...
  const find = (arr, isLeft) => {
    ...
    for (let i = 0; i < arr.length; i++) {
      ...
      if (sum * 2 > total) {
        ...
        if (left.some(item => item.includes(2 * sum - total))) {

          if (left.length > 1) {
            return true
          } else {
            const findIndex = left[0].findIndex(item => item === sum * 2 - total)
