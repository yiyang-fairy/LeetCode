...
const minimumRefill = function (plants, capacityA, capacityB) {
  ...
  while (i <= j) {
    if (i === j) {
      const max = Math.max(remainA, remainB)
      if (max < plants[i]) {
        ans++
      }
      break
...
console.log('测试用例1', minimumRefill([2, 2, 3, 3], 5, 5));  //1
console.log('测试用例2', minimumRefill([2, 2, 3, 3], 3, 4));  //2
console.log('测试用例3', minimumRefill([5], 10, 8));  //0